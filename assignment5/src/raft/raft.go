...
func (rf *Raft) AppendEntries(args AppendEntriesArgs, reply *AppendEntriesReply) {
	//DPrintf("Node %v receive RPC from %v with term %v and its current term is %v", rf.me, args.LeaderID, args.Term, rf.currentTerm)
	...
	DPrintf("Node %v received valid RPC from %v", rf.me, args.LeaderID)
	...
	DPrintf("Node %v received valid entries from %v, %v", rf.me, args.LeaderID, args.Entries)
	reply.Success = true
...
func (rf *Raft) Start(command interface{}) (int, int, bool) {
	...
	if isLeader == true {
		index = len(rf.log)
		DPrintf("Node %v received a new command at term %v index %v, %v", rf.me, term, index, command)
...
func startElection(raft *Raft, wg *sync.WaitGroup) {
	...
	if totalVotes*2 > totalPeers { // Win election with majority votes
		raft.transferToLeader()
		raft.Start(0) // no-op command
	...
	for server := 0; server < len(raft.peers); server++ {
		...
		go func(s int) {
			...
			if raft.sendAppendEntries(s, args, reply) == false {
				//DPrintf("Node %v failed to send RPC to %v", raft.me, s)
				return
			}
...
func (rf *Raft) checkCommitIndex(matchIndices []int, term int) {
	...
	//DPrintf("Node %v has commit matches %v", rf.me, matchIndices)
...
func (rf *Raft) applyLog() {
	for {
		...
		for commitIndex > rf.lastApplied {
			...
			rf.mu.Unlock()
			//DPrintf("Node %v aboue to apply log for command %v at index %v", rf.me, msg.Command, msg.Index)
			rf.applyCh <- msg
			DPrintf("Node %v done applying log for command %v at index %v", rf.me, msg.Command, msg.Index)
		}
...
func (rf *Raft) transferToLeader() {
	...
	DPrintf("Node %v becomes leader", rf.me)
